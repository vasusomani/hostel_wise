...
class HistoryCleaning extends StatefulWidget {
  HistoryCleaning(this.secKey);
  final String secKey;

  @override
  _HistoryCleaningState createState() => _HistoryCleaningState();
}

class _HistoryCleaningState extends State<HistoryCleaning> {
  List<Map<String, dynamic>> cleaningHistory = [];

  @override
  void initState() {
    super.initState();
    _fetchCleaningHistory();
  }
  ...
  Future<void> _fetchCleaningHistory() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        cleaningHistory = List<Map<String, dynamic>>.from(responseBody);
      });
    } else {
      print("Failed to fetch cleaning history. ${response.statusCode}");
    }
  }
